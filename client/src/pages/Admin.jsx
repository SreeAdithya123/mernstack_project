...
function Analytics() {
  ...
  return (
    <section>
      <h2 className="text-sm font-semibold text-slate-700">Ticket analytics</h2>
      <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
          <p className="font-medium">Total tickets: {stats.total}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
          <p className="font-medium">By status</p>
          <p className="text-xs text-slate-500">{breakdown(stats.byStatus)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
          <p className="font-medium">By sentiment</p>
          <p className="text-xs text-slate-500">{breakdown(stats.bySentiment)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
          <p className="font-medium">By category</p>
          <p className="text-xs text-slate-500">{breakdown(stats.byCategory)}</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section>
      ...
      <div className="mt-2 overflow-hidden rounded-lg border border-slate-200 bg-white">
        ...
        {profiles.length === 0 && <p className="p-3 text-xs text-slate-400">No users yet.</p>}
      </div>
    </section>
  );
}
...
function KBManagement() {
  ...
  return (
    <section>
      <h2 className="text-sm font-semibold text-slate-700">Knowledge base articles</h2>
      {error && <p className="mt-1 text-xs text-red-600">{error}</p>}

      <form onSubmit={submit} className="mt-2 space-y-2 rounded-lg border border-slate-200 bg-white p-3">
        ...
        <div className="flex gap-2">
          ...
          {editingId && (
            <button type="button" onClick={cancelEdit} className="rounded-md border border-slate-300 px-3 py-1.5 text-xs text-slate-600">
              Cancel
            </button>
          )}
        </div>
      ...
      <ul className="mt-3 space-y-2">
        {articles.map((a) => (
          <li key={a.id} className="rounded-lg border border-slate-200 bg-white p-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(a)} className="text-xs text-indigo-700 hover:underline">
                  Edit
                </button>
        ...
        {articles.length === 0 && <p className="text-xs text-slate-400">No articles yet.</p>}
      </ul>
    </section>
  );
}
