// Trigger-invoked (pg_net, shared secret) on every customer ticket_messages
// INSERT that isn't the ticket's first message and isn't already resolved
// (filtering is done in the trigger function itself - see migration 014).
// Operates on the raw message body; the LLM handles non-English input
// natively, so this doesn't need to wait on translate-message.
...
const SYSTEM_PROMPT = `You are a strict classifier for a customer support platform. You are given the LATEST customer message in an ongoing support ticket thread. Decide if this message ONLY expresses thanks, acknowledgement, or confirmation that the issue is resolved - with NO new question, problem, request, or open concern (even a mild one).

Be conservative: if there is ANY ambiguity, any hint of a remaining issue, or any new information, answer false. Only answer true for unambiguous closing remarks like "thanks, that fixed it!", "perfect, appreciate the help", "all good now, thank you".

Respond with ONLY a single JSON object - no markdown fences, no commentary - in exactly this shape:
{"is_closing": true|false, "reason": "one short sentence"}`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: ticket, error: ticketErr } = await admin.from('tickets').select('id, status').eq('id', message.ticket_id).single();
    if (ticketErr || !ticket) throw new Error('ticket not found');
    if (ticket.status === 'resolved') {
      return new Response(JSON.stringify({ skipped: 'already resolved' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const verdict = await chatJson(
      ...
      ['is_closing', 'reason'],
      0,
      (parsed) => ({ isClosing: Boolean(parsed.is_closing), reason: String(parsed.reason).trim() })
    );

    if (!verdict.isClosing) {
      return new Response(JSON.stringify({ closed: false }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const { error: updateErr } = await admin
      ...
      .update({
        status: 'resolved',
        auto_closed_by_ai: true,
        resolution_summary: `Auto-closed by AI: ${verdict.reason}`,
      })
      .eq('id', ticket.id);
    if (updateErr) throw new Error(`failed to auto-close: ${updateErr.message}`);

    return new Response(JSON.stringify({ closed: true, reason: verdict.reason }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (err) {
    console.error(err);
